...
import { FeeLevel, ContractExecutionBlockchain, TransactionType, TransactionState, Blockchain, CustodyType, Operation, ListTransactionsOrderEnum, WalletMetadata, AccountType, TokenStandard, ScaCore, GetWalletsOrderEnum, GetWalletSetsOrderEnum, EvmBlockchain, WalletsWithBalancesData, TokenBlockchain, Wallet, Balance } from '../clients/developer-controlled-wallets';
import * as _shared_core from '../clients/core';
import { ClientParams as ClientParams$1, Common, WithIdempotencyKey, FeeConfiguration, Pagination, RequestTestnetTokensInput, CreateMonitoredTokensInput, UpdateMonitoredTokensInput, ListMonitoredTokensInput, DeleteMonitoredTokensInput, UpdateMonitoredTokensScopeInput, CreateSubscriptionInput, UpdateSubscriptionInput } from '../clients/core';
 ...
 * Solana devnet.
 */
declare const SOL_DEVNET: {
    readonly blockchainId: "SOL-DEVNET";
    readonly cluster: "devnet";
};
/**
 * Maps accepted `chain` inputs (Circle `Blockchain` wire values and Solana
 * cluster names per https://solana.com/docs/references/clusters) to the
 * Circle `Blockchain` wire value the Signing API expects.
 */
declare const SolanaBlockchain: {
    readonly SOL: "SOL";
    readonly "mainnet-beta": "SOL";
    readonly "SOL-DEVNET": "SOL-DEVNET";
    readonly devnet: "SOL-DEVNET";
};
type SolanaBlockchain = keyof typeof SolanaBlockchain;
...
/**
 * Represents the input to sign a transaction from a specified developer-controlled wallet.
 */
...
interface GetTransactionInput extends Common {
    ...
    /**
     * When provided, polls the transaction until it reaches (or is past) the
     * requested state along the success pipeline
     * `INITIATED → CLEARED → QUEUED → SENT → CONFIRMED → COMPLETE` — e.g.
     * `waitForState: 'SENT'` resolves when the current state is `SENT`,
     * `CONFIRMED`, or `COMPLETE`.
     *
     * If the transaction enters a terminal failure state (`CANCELLED`, `DENIED`,
     * `FAILED`, `STUCK`) before reaching the requested state, the returned
     * promise rejects.
     *
     * When omitted (default), the transaction is fetched once with no polling.
     */
    waitForState?: TransactionState;
    /**
     * Polling interval in milliseconds used when `waitForState` is set.
     * Must be greater than 0. Defaults to 1000.
     */
    pollingInterval?: number;
    /**
     * Abort signal used to cancel polling when `waitForState` is set. When the
     * signal aborts, the in-flight delay rejects with the signal's reason.
     */
    signal?: AbortSignal;
}
/**
...
type ListTransactionsInput = {
    ...
    /**
     * Filters transactions based on the custody type.
     */
...
type WalletWithBalances = EOAWalletWithBalances | SCAWalletWithBalances;
type GetWalletsWithBalancesResponseData = Omit<WalletsWithBalancesData, 'wallets'> & {
...
declare class CircleDeveloperControlledWalletsClient {
     ...
     *
     * Pass `waitForState` to poll until the transaction reaches or passes the
     * requested state along the success pipeline
     * `INITIATED → CLEARED → QUEUED → SENT → CONFIRMED → COMPLETE` (e.g.
     * `waitForState: 'SENT'` also resolves on `CONFIRMED` or `COMPLETE`).
     * Rejects if the transaction enters a terminal failure state (`CANCELLED`,
     * `DENIED`, `FAILED`, `STUCK`) first. Without `waitForState` the transaction
     * is fetched once with no polling.
     * @example
     * ```
     ...
     *
     * // Wait until on-chain (resolves on SENT or any later state)
     * const response = await client.getTransaction({
     *   id: '9fcb2e86-dec2-4226-81d1-4dbad429278c',
     *   waitForState: 'SENT',
     *   pollingInterval: 2000,     // optional, default 1000 ms
     ...
     * @category Wallets
     */
    getWalletsWithBalances(input: GetWalletsWithBalancesInput): Promise<_shared_core.TrimDataResponse<{
        data: GetWalletsWithBalancesResponseData;
    }>>;
    /**
...
interface CreateTransactionSignerInput<TAddress extends Address = Address> {
    client: CircleDeveloperControlledWalletsClient;
    address: TAddress;
    chain: SolanaBlockchain;
}
/**
 * Create a `@solana/kit` `TransactionPartialSigner` backed by a Circle
 * developer-controlled wallet.
 *
 * @category Signing
 * @throws {UnsupportedChainError} if `chain` is not a Circle Solana
 *   `Blockchain` value or a supported Solana cluster alias.
 */
declare function createTransactionSigner<TAddress extends Address = Address>({ client, address, chain, }: CreateTransactionSignerInput<TAddress>): TransactionPartialSigner<TAddress>;

/** Thrown when the requested chain is not supported. */
declare class UnsupportedChainError extends Error {
}

export { SOL, SOL_DEVNET, SolanaBlockchain, UnsupportedChainError, createTransactionSigner };
export type { CreateTransactionSignerInput };
